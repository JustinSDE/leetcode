...
<!--If you want to use the English description, use <p>Determine if a&nbsp;9x9 Sudoku board&nbsp;is valid.&nbsp;Only the filled cells need to be validated&nbsp;<strong>according to the following rules</strong>:</p>
...
class Solution:
    def isValidSudoku(self, board):
        ...
        """
        for row in board:
            tmp = list(filter(lambda x : x != '.', row))
            if len(set(tmp)) < len(tmp):
                return False
        for col in zip(*board):
            tmp = list(filter(lambda x : x != '.', col))
            if len(set(tmp)) < len(tmp):
                return False
        for i in range(3):
            for j in range(3):
                tmp = list(filter(lambda x : x != '.', [__ for _ in list(zip(*board[i*3:(i+1)*3]))[j*3:(j+1)*3] for __ in _]))
                if len(set(tmp)) < len(tmp):
                    return False
        return True
```
...
Question: [LeetCode](https://leetcode.com/problems/valid-sudoku/description/)  /  [LeetCode中国](https://leetcode-cn.com/problems/valid-sudoku/description/)

Solution: [LeetCode](https://leetcode.com/articles/valid-sudoku/)  /  [LeetCode中国](https://leetcode-cn.com/articles/valid-sudoku/)