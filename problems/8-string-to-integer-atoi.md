...
<!--If you want to use the English description, use <p>Implement <code><span>atoi</span></code> which&nbsp;converts a string to an integer.</p>
...
使用正则表达式和自带的类型转换完成。不过，该题本意应该是想考查底层实现。
...
class Solution:
    def myAtoi(self, str):
        ...
        if num:
            return min(max(-2**31, int(num[0])), 2**31-1)
        else:
            return 0
```
