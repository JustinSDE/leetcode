...
<!--If you want to use the English description, use <p>We are given two arrays <code>A</code> and <code>B</code> of words.&nbsp; Each word is a string of lowercase letters.</p>
...
Language: **python3**  /  Runtime: 1424 ms
...
class Solution:
    def wordSubsets(self, A, B):
        ...
        bcnt = {}
        for b in B:
            for bk, bv in collections.Counter(b).items():
                bcnt[bk] = max(bcnt.get(bk, 0), bv)
        res = []
        for a in A:
            acnt = collections.Counter(a)
            for k in bcnt:
                if acnt.get(k, 0) < bcnt[k]:
                    break
            else:
                res.append(a)
        return res
...
Question: [LeetCode](https://leetcode.com/problems/word-subsets/description/)  /  [LeetCode中国](https://leetcode-cn.com/problems/word-subsets/description/)

Solution: [LeetCode](https://leetcode.com/articles/word-subsets/)  /  [LeetCode中国](https://leetcode-cn.com/articles/word-subsets/)