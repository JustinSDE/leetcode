...
class Solution:
    def shortestCompletingWord(self, licensePlate, words):
        ...
        """
        res = []
        lpc = dict(collections.Counter(filter(lambda c: c.isalpha(), licensePlate.lower())))
        for word in words:
            wordc = dict(collections.Counter(word))
            for c, n in lpc.items():
                if c not in wordc or wordc[c] < n:
                    break
            else:
                res.append(word)
        res.sort(key=lambda w: len(w))
        return res[0]
...
Question: [LeetCode](https://leetcode.com/problems/shortest-completing-word/description/)  /  [LeetCode中国](https://leetcode-cn.com/problems/shortest-completing-word/description/)

Solution: [LeetCode](https://leetcode.com/articles/shortest-completing-word/)  /  [LeetCode中国](https://leetcode-cn.com/articles/shortest-completing-word/)